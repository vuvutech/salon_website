...
<footer className='bg-gray-100 dark:bg-gray-950 lg:grid lg:grid-cols-5 relative'>
			...
			<div className="inset-0 absolute opacity-15  " style={{  backgroundImage: "url('../../hero-bg2.png')" }}></div>
						...
						<div className='grid grid-cols-1 gap-4 sm:grid-cols-4  dark:text-gray-200'>
							<Link href={`${services[0].services[3].url}`}>
								...
								<ul className='mt-6 space-y-4 text-xs'>
									{services[0].services[3].services.map(
										(service) => (
											<li
												class='normal-case'
												key={service.name}>
							...
							<Link href={`${services[0].services[1].url}`}>
								...
								<ul className='mt-6 space-y-4 text-xs'>
									{services[0].services[1].services.map(
										(service) => (
											<li
												class='normal-case'
												key={service.name}>
							...
							<Link href={`${services[0].services[2].url}`}>
								...
								<ul className='mt-6 space-y-4 text-xs'>
									{services[0].services[2].services.map(
										(service) => (
											<li
												class='normal-case'
												key={service.name}>
							...
							<Link href={`${services[0].services[0].url}`}>
								...
								<ul className='mt-6 space-y-4 text-xs'>
									{services[0].services[0].services.map(
										(service) => (
											<li
												class='normal-case'
												key={service.name}>
				...
				<div className='mt-12 border-t border-gray-100 pt-12'>
					<div className='sm:flex sm:items-center sm:justify-between'>
						...
						<p className='mt-8 text-xs text-gray-500  dark:text-gray-200 sm:mt-0'>
							&copy; {currentYear}. Elsie Hair Salon &#8212; All
							rights reserved.
						</p>
					</div>
				</div>
